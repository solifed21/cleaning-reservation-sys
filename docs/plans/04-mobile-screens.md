...
### 포함
- 인증(로그인) + 역할 선택(최초 1회)
- 요청자: 예약(booking) 생성, 내 예약 목록/상세, 취소
- 제공자: 일감(대기 예약) 목록, 수락/거절, 진행/완료 처리
- 예약 기반 메시지(폴링)
- 리뷰 작성/조회
- 설정(로그아웃)
...
> 역할(role)은 서버의 `users.role` 기반(02 문서). 최초 role-select에서 서버에 반영.
...
- 탭 화면은 “리스트/허브” 중심
- 생성/상세/편집은 Stack
- `bookingId`를 URL param으로 공유 화면 진입
...
> NOTE: API 문서(03)의 URL은 `/api/v1/bookings`로 정의되어 있으므로, 화면/컴포넌트 네이밍도 booking을 사용합니다.
...
- 입력: `roomType`, `roomSize`, `services[]`, `duration(시간)`
...
**데이터**
- `GET /api/v1/bookings?status=pending` (서버에서 cleanerView 지원 시 추가)
...
**경합 처리(중요)**
- 이미 다른 cleaner가 수락했다면 `CONFLICT` 에러를 표준 처리:
...
- `GET /api/v1/cleaners/me`, `PATCH /api/v1/cleaners/me` (03 문서)
...
- 앱 background 시 polling 중지(visibility/appstate 연동)
- 전송은 optimistic update
- 이미지 메시지는 Post‑MVP (현재 스키마/엔드포인트는 확장 가능)
...
> DB enum(02): `pending | confirmed | in_progress | completed | cancelled`
...
- 네트워크 실패
  - 공통 ErrorView + 재시도
  - 리스트는 pull-to-refresh 제공
- 중복 제출 방지
  - mutation 진행 중 CTA disabled
- 권한 에러
  - 401 → 로그인 화면으로
  - 403 → “권한이 없습니다” + 이전 화면
- 경합(CONFLICT)
  - 제공자 수락 경쟁 발생 시 토스트 + 리스트 갱신
