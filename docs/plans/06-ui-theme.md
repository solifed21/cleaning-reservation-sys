...
### 0.3 제품 톤(시각) 3원칙
- **White space first**: 여백이 곧 ‘청결감’
- **Border over shadow**: 그림자는 최소, 경계선/계층으로 구조화
- **Meaningful color only**: 컬러는 ‘장식’이 아니라 ‘의미(상태/행동)’ 전달에만 사용
...
- 버튼 라벨: **동사+목적어**(예: “예약 요청하기”, “취소하기”, “주소 수정하기”)
...
UX 규칙
- Primary는 한 화면에 1개가 원칙(동등한 Primary 2개 금지)
- destructive는 단독으로 배치(근처에 primary 두지 않기)
...
폼 레이아웃
- 모바일: 필드 간 12~16px
- 제출 버튼: 화면 하단 고정(옵션), 키보드 올라올 때 가림 방지
...
### 4.10 Empty / Skeleton (품질 기본)
- 빈 상태는 “왜 비었는지 + 다음 행동”
  - 예: “아직 예약이 없어요. 지금 예약을 만들어볼까요?”
- 로딩은 스켈레톤(리스트/카드)
- 스피너만 덩그러니 금지(사용자가 ‘무엇이 로딩 중인지’ 알 수 있어야 함)
...
## 11) Figma(또는 디자인 산출물) 구성 권장

- Pages
  - `00 Foundations` (Color/Type/Space/Radius)
  - `01 Components` (Button/Input/Card/Badge…)
  - `02 Patterns` (예약 플로우, 취소/환불, 빈 상태)
  - `03 Screens` (실제 화면)
...
- 핵심 원칙
  - 토큰과 컴포넌트가 먼저, 스크린은 마지막
  - 스크린에서 임의 스타일 생성 금지(반드시 컴포넌트/토큰으로 환원)
